  ...
  <view v-if="visibleSync" class="ei-calendar" :class="{ 'ei-calendar--drawer': drawer, 'is-active': active}">
    ...
    <view class="ei-calendar__container">
      ...
      <view class="ei-calendar__container__footer" v-if="!disabled">
        <button style="background-color: #B0E0E6; color: black;" type="primary" @click="doSubmit">确定</button>
        <button style="background-color: #FFE4E1; color: black;" @click="showUserEvents">显示当天事件</button>
        <button style="background-color: #FFF8DC; color: black;" @click="showAllEvents">显示全部事件</button>
        <button style="background-color: #6789AF; color: black;" @click="manageEvents">(选择当天完成的事项)</button>
		<button style="background-color: #FF6357; color: black;" @click="showdoneEvents">(查看完成的所有事项)</button>
      </view>
    </view>
  </view>
</template>
...
export default {
  ...
  data() {
    return {
	  addedEvents: [], // 已添加的事项数组
	  eventsByDate: {}, // 按日期存储的事项对象
	  doneEvents: {},//已完成的事项
      visibleSync: false,
      active: false,
      closeTimer:null,
 ...
 doSubmit() {
   ...
   if (value.length === 1) {
     uni.showModal({
       title: '请输入今日需要做的事项时间与事项内容',
       content: '',
       showCancel: true,
 		 editable: true,
       success: (res) => {
         if (res.confirm) {
           const event = res.content; // 用户输入的内容
           if (event) {
             const formattedDate = EDate.format(new Date(value[0]), this.format); // 格式化所选日期
             if (!this.eventsByDate[formattedDate]) { // 如果该日期没有对应的事件数组，创建一个空数组
               this.$set(this.eventsByDate, formattedDate, []);
             }
             this.eventsByDate[formattedDate].push(event); // 添加事件到对应日期的事件数组中
 			 uni.setStorageSync('eventsByDate', this.eventsByDate);
			 }
         } else if (res.cancel) {
           console.log('用户点击取消');
         }
     ...
     uni.showModal({
       title: '请输入这几天需要做的事项时间与事项内容',
       editable: true,
       success: (res) => {
         if (res.confirm) {
           const event = res.content;
           if (event) {
             dateArray.forEach(date => {
               const formattedDate = EDate.format(date, this.format);
   ...
   showUserEvents() {
     const date = this.getResultValue();
     if (this.eventsByDate[date]) {
       uni.showModal({
         title: `${date}的事件`,
         content: this.eventsByDate[date].join('\n'),
       });
     } else {
       uni.showToast({
         title: '当天无事件',
   ...
   showAllEvents() {
     let allEvents = '';
     for (const [date, events] of Object.entries(this.eventsByDate)) {
       allEvents += `${date}:\n${events.join('\n')}\n\n`;
     }
     if (allEvents) {
       uni.showModal({
   ...
   manageEvents() {
     ...
     if (this.eventsByDate[date]) {
       uni.showActionSheet({
         itemList: this.eventsByDate[date],
         success: (res) => {
           if (!res.cancel) {
             const index = res.tapIndex;
             const event = this.eventsByDate[date][index];
             // 在这里进行删除或标记已完成操作
             console.log('删除或标记已完成事项：', event);
             // 将被删除的事件保存到 doneEvents 中
            if (!this.doneEvents[date]) {
                this.$set(this.doneEvents, date, []);
            }
            this.doneEvents[date].push(event);
            // 删除事项
            this.eventsByDate[date].splice(index, 1);
			if (this.eventsByDate[date].length === 0) { // 如果删除后该日期对应的事件为空
				delete this.eventsByDate[date]; // 从事件对象中移除该日期
			}
            uni.setStorageSync('eventsByDate', this.eventsByDate); // 保存修改后的事件对象
            uni.setStorageSync('doneEvents', this.doneEvents); // 保存已完成的事件
           }
         }
       });
     } else {
   ...
   showdoneEvents() {
     let doneEvents = uni.getStorageSync('doneEvents');
       if (doneEvents) {
         let modalContent = '';
         for (let date in doneEvents) {
           modalContent += `日期：${date}\n已完成的事件：\n`;
           doneEvents[date].forEach((event, index) => {
             modalContent += `${index + 1}. ${event}\n`;
           });
         }
         uni.showModal({
           title: '已完成的事件',
           content: modalContent || '没有已完成的事件',
           showCancel: false,
           confirmText: '确定'
         });
       } else {
   ...
   initVisible() {
       ...
       if (this.visible) {
         // 从本地存储中获取事件数据
         const storedEventsByDate = uni.getStorageSync('eventsByDate');
         if (storedEventsByDate) {
           this.eventsByDate = storedEventsByDate;
         }
         this.initValue();
       }
       setTimeout(() => {
